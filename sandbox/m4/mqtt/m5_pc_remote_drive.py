...
Authors: David Fisher and Craig McGee II, Chris Fusaro, Jacob Back.
...
def main():
    ...
    forward_button['command'] = lambda: drive_forward(mqtt_client,
                                                 left_speed_entry,
                                          right_speed_entry)
    root.bind('<Up>', lambda event: drive_forward(mqtt_client,
                                                 left_speed_entry, right_speed_entry))

    left_button = ttk.Button(main_frame, text="Left")
    left_button.grid(row=3, column=0)
    left_button['command'] = lambda: spin_left(mqtt_client,
                                                     left_speed_entry,
                                                     right_speed_entry)
    root.bind('<Left>', lambda event: spin_left(mqtt_client,
                                                  left_speed_entry,
                                                  right_speed_entry))
    ...
    right_button['command'] = lambda: spin_right(mqtt_client,
                                                 left_speed_entry, right_speed_entry)
    root.bind('<Right>', lambda event: spin_right(mqtt_client,
                                                 left_speed_entry, right_speed_entry))

    back_button = ttk.Button(main_frame, text="Back")
    back_button.grid(row=4, column=1)
    back_button['command'] = lambda: drive_backward(mqtt_client,
                                                 left_speed_entry,
                                                 right_speed_entry)
    root.bind('<Down>', lambda event: drive_backward(mqtt_client,
                                                  left_speed_entry,
                                                  right_speed_entry))

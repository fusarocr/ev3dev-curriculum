# !/usr/bin/env python3
"""
In this module you will use the touch sensor to make arm movements.  
Instead of writing code from scratch you will
...
# Can you see what the robot does and explain what each line of code is doing?
# Talk as a group to make sure.
...
def arm_calibration(arm_motor, touch_sensor):
    """
    Runs the arm up until the touch sensor is hit then back to the bottom
    again, beeping at both locations.Once back at in the bottom position,
    gripper open, set the absolute encoder position to 0.  You are calibrated!
    The Snatch3r arm needs to move 14.2 revolutions to travel from the touch
    sensor to the open position.
    ...
    #   Command the arm_motor to run forever in the positive direction at
    # max speed.
    #   Create an infinite while loop that will block code execution until
    # the touch sensor's is_pressed value is True.
    #     Within that loop sleep for 0.01 to avoid running code too fast.
    #   Once past the loop the touch sensor must be pressed. So stop the arm
    # motor quickly using the brake stop action.
    #   Make a beep sound
    #   Now move the arm_motor 14.2 revolutions in the negative direction
    # relative to the current location
    #     Note the stop action and speed are already set correctly so we don't
    #  need to specify them again
    #   Block code execution by waiting for the arm to finish running
    #   Make a beep sound
    #   Set the arm encoder position to 0 (the last line below is correct to do
    # that, it's new so no bug there)

    # Code that attempts to do this task but has MANY bugs
    # (nearly 1 on every line).  Fix them!
...
def arm_up(arm_motor, touch_sensor):
    ...
    # Command the arm_motor to run forever in the positive direction at max
    # speed.
    # Create a while loop that will block code execution until the touch
    #  sensor is pressed.
    #   Within the loop sleep for 0.01 to avoid running code too fast.
    # Once past the loop the touch sensor must be pressed. Stop the arm motor
    # using the brake stop action.
    # Make a beep sound
...
def arm_down(arm_motor):
    ...
    #
    # Observations you should make, the TouchSensor is easy to use,
    # but the motor commands are still a little bit
    #   tricky.  It is neat that the same motor API works for both the wheels
    # and the arm.
